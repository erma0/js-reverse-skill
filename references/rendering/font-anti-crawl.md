...
| **伪字体（无字体文件）** | **全站无 woff/woff2/`@font-face`/`FontFace`**，"乱码"是**码位单表替换**，还原表以前端常量形式明文存在（`{ glyphChar: '0', … }`） | 一条正则提表 + 查表，**不进本节 §3 的字体解析链** |
...
- **伪字体（无字体文件）形态的反向信号**：抓包谱里**没有任何字体资源**、HTML/CSS 无 `@font-face`、trace 无
  `FontFace`/`fonts.add`，但业务数据字段仍是乱码；同时落盘 JS 里能直接搜到形如
  `XXX_MAP={'<非 ASCII 单字符>':'0', …}` 的明文对象字面量（10 项/数字双射）。
  另一个判别细节：乱码字符**不在 PUA 区**而是散落真实文字区块（孟加拉文/古吉拉特文/西里尔文扩充-B 等，
  如题17 样本 U+09A7·U+0AA4–U+0AB8·U+A66E）——
  真字体反爬通常占 PUA，占真实文字区块的多是"借字符做单表替换"。

## 3. 还原路径

**先做三问分诊，再选还原链**（三问都只花几十秒，能省掉整条 fontTools 工具链）：

1. **有字体资源吗？** 抓包谱里有 `.woff/.woff2/.ttf`、HTML/CSS 有 `@font-face`、或数据响应内嵌 base64 字体？
   全否 ⇒ 不是真字体反爬，走第 3 问。
2. **字体是静态还是动态？** 两次取证会话比对字体 URL/内容 hash；响应内嵌 base64 直接判动态。
3. **站方 JS 里有明文表吗？** 在落盘 JS 里搜 `_MAP=`/`fontMap`/`decode.*Font` 与非 ASCII 键的对象字面量；
   命中 ⇒ 提表查表即完成还原（注意混淆器对**非 ASCII 键的对象字面量常常直接留明文**，
   所以"文件是 jsjiami/obfuscator 家族"≠"表也被保护"，先 `search_js.js` 搜关键词再决定是否跑反混淆流水线）。

1–3 命中"伪字体"后**不要**继续本节字体解析；下面步骤适用于第 1 问答"是"的真字体形态：
...
- **题型命名不是实现形态依据**：题目/标签写"字体加密""font 混淆"不构成上 fontTools/brotli/字形指纹的理由；
  以 §3 三问分诊的实测结果为准（码上爬题17 实证：全站零字体资源，还原表是站方 JS 明文常量）。
- **伪字体形态的表不要硬编码进交付**：这类表同样随站点改版变化，交付应运行时重拉脚本 + 提表 + 记录 sha256，
  并校验"10 项 / 数字双射 / 解码结果为纯数字"，出现表外码位**硬失败**而不是静默产出 `NaN` 参与求和。
...
- 内容还原型（请求侧零签名）的 Step 2 豁免判据：`SKILL.md` §4.4 例外 3
- 伪字体形态实证（无字体文件、表在 JS 明文常量）：`cases/unicode-codepoint-substitution-no-font-file-mashangpa-p17.md`
- 真字体动态映射实证（响应内嵌 base64 TTF + `glyf` 指纹）：`cases/yuanrenxue-match7-dynamic-font.md`
