...
比"定期更新"更隐蔽的动态性：**多个资源之间存在解密配套关系**，由服务端在同一次渲染中配套下发（match10 实测：瑞数 v3 变种的 boot 引导器与 /api2/10 下发的密文字符串表）。这类资源单独看都可能"解析正常"，跨渲染混用时解密结果静默损坏：

- 识别信号：引导代码（boot/组装器）内嵌会话绑定值；密文资源由特定接口在同一次页面渲染中下发；两次拉取 HTML 后引导代码 hash 不同。
- 混用后果（match10 实测）：旧渲染的 boot + 新渲染的 api2 密文 → 流式解密在固定字节处错乱 → 拼出残缺 core → 签名动态段全零（零值填充串）→ 服务端 400。错误表现与签名算法错误、通道层拦截完全一致，极难排查。
- 纪律：
  1. 最终入口每次运行**新鲜 GET 入口 HTML**，从同次响应中提取配套的全部资源（内联脚本、meta challenge、动态密文），禁止复用任何一次历史渲染的快照与本次资源拼接。
  2. 资源间存在引用关系时（boot 解密 api2 密文、组装器消费字符串表），在 resource-manifest 中用 `dependsOn` 记录配套关系，并标注 `pairedWithin: "single-render"`。
