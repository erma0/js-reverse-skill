...
| **数据绑定会话基线（归因陷阱）** | 数据/答案随请求变化，诱导归因"平台数据按周期重置"或"IP 限流" | **先验 session 基线**：固定 sessionid 重放，数据恒定 = 数据绑定会话（match9 实测：匿名 session 每次数据不同、固定 session 答案 27848571 不变）；答题类先确认 session 基线再归因 |
...
- 答题/数据类站点出现"数据变化、答案不对"时，**先固定 sessionid 重放**验证数据是否绑定会话，再归因"平台重置/IP 限流/风控"（match9 实测"数据周期重置"是假象，真实机制是数据绑定 session；会话 cookie 也可能由 API 响应 set-cookie 下发而非页面）
