...
| 8. 取证被目标站内核级检测阻断 | 目标请求在取证浏览器（Firefox 内核）持续 4xx 且已定位到 UA 之外的内核级检测（`eval.toString()` 分支、Error.stack 格式等，改 UA / `--ua` 无效） | 进入状态机 `BLOCKED_FORENSIC`（SKILL.md 第 4 节）：输出卡点与检测证据对齐用户——用户提供真实浏览器 cURL/HAR 走 MATERIALS_FALLBACK，或用户确认降级（义务同阻塞点 #5 例外：经验沉淀与最终总结写明 Step 2 缺失原因，REAL_VERIFY 不可豁免）。定位方法见 `references/env/env-detect-bypass.md` 内核级差异检测；**不得**静默跳过 Step 2 直接补环境，也**不得**无限重试取证 |
