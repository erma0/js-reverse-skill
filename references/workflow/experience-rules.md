...
match10 实测：sandbox 预填"看起来完整"的 `$_ts` 快照（tsFull）后，rs.js 检测到状态已存在走旁路，静默跳过动态初始化（不发 api2 请求），链路残缺且无任何报错。黑盒执行的价值在于让引导代码按真实分支自然驱动——预填快照相当于替目标代码做了分支决策，与缓存复用（规则 22）同属改变执行语义。正确做法：空状态起步，按浏览器加载顺序喂资源（如 rs.js → api2 → boot），每步初始化由目标代码自己完成；确需注入状态时，先在 trace 中确认该状态的真实写入时机早于当前执行步骤，否则一律不预填。
...
| `cases/yuanrenxue-match10-ruishu3-replay-defense.md` | 规则 24 实战验证（预填 $_ts 快照致 rs.js 走旁路）+ 反模式 16/20/21 实战验证（插桩 while(1) 禁令 / VM 卡死转投浏览器 / 外部失败误归因通道层）+ 会话配套资源（dynamic-resource.md 专节）+ 元素语义真实化（env-object-model.md 元素语义章节） |
