...
原始 Trace
  ...
  -> 真实请求
```
用户不需要额外提出“把 Trace 和当前环境一一对比”；只要存在 Trace 且实现路径为 B（JS 沙箱）/ C（WASM）/ D（补环境），该循环就是默认流程。**路径 A（纯算法，无 JS 沙箱执行环境）豁免本对拍循环**——纯算交付不存在运行时环境差异可对拍，直接进入 REAL_VERIFY ≥5 次真实请求验证；若纯算实现在后续真实验证中失败并升级到 B/C/D 路径，则本循环随之变为强制。
