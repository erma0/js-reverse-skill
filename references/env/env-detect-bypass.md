...
**三种命中形态与对应证据**（判"该不该降级"时按形态取证据，别只盯状态码）：

| 形态 | 症状 | 可交付的降级证据 |
|---|---|---|
| A 报错型 | 目标 JS 抛 `SyntaxError: illegal character` 等，签名不生成 | 异常栈 + 分支判定项采样（上方 `run_js` 一次采齐） |
| B 拒绝型 | 目标请求发出但 400/403，与 UA/Cookie 无关 | `target-hits`/`capture.json` 的拒绝包 + `--ua` 覆盖后逐轮同构 |
| C **静默断链型** | **目标请求一次都没发出**，页面脚本看似全加载（SDK 200、上报接口 200） | **跨引擎对照**：同一份落盘 JS 在取证内核停在 SDK 状态机中段、在 Node(V8) 沙箱走通全链；断点位置由 storage 键位序列钉住（见 `ruyitrace-cheatsheet.md` §6.3） |

C 形态最容易误判成"接口路径猜错 / `--targets` 写错 / 还差一轮浏览器"，实证来源（mashangpa 题16）：
定制 Firefox 取证内核 3 轮（含 `--ua` 覆盖为 Chrome UA、含暖 profile 复现"刷新后即出"的用户观察）+
日志采集内核 1 轮，**每轮 20 包、目标 data 请求 0 次**；`--ua` 覆盖后包谱逐字节同构 ⇒ 排除 UA 串判定；
而本地 vm 沙箱跑同一份 `PcSign.js` 能产出被服务端接受的签名 ⇒ 引擎级分歧成立，降级为
「沙箱直调落盘 JS 取 writer 真值 + 服务端接受与否作 oracle」，Step 1 缺失在 `entry-chain.md` 与
`最终项目总结.md` 双处显式声明。

**判 C 的前置纪律**：先把 `--targets` 的业务拒绝（HTTP 200 包业务码）排除掉（trace-flow 第④坑），
再确认"真的一个请求都没发"，而不是取证提前收尾；两者症状相近，结论方向相反。
