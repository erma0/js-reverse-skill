...
2. 文字题用 OCR（ddddocr `det=True` 检测模式优先）；物体/图标题用目标检测、模板匹配或 VLM。
3. 尽量同时返回截图像素坐标和元素相对坐标。
4. 判断厂商是否要求有序点击、多轮题面或点击后确认。
5. 中文文字点选必须保留原始字符和目标顺序。
6. 点击时序/坐标抖动用 `generate_motion_track.py --mode click` 生成（厂商采集形态以成功样本明文为准，见 `captcha-motion-encryption.md`）。
