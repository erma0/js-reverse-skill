...
> 本文件归档 2.3.87 及更早的历史版本；主文件 CHANGELOG.md 仅保留最近若干版本（2.3.88 起）。
...
反模式清单增长到 27 个编号（24 条实条）后扫视与查表成本过高（文件内"封顶线"规则也早已超线）。本轮按文件自身"同根因合并优先"规则完成 5 组合并，沿用 5/15/21 既有指针模式：**编号全部保留、内容并入主条目、外部引用零破坏**。实条 24 → 19。

- **反模式 2 → 1**（交付红线）：「Cookie/密钥硬编码」并入「浏览器 cookie 当最终方案」，统一为"交付物依赖会失效的静态值而非协议动态还原"（浏览器产物 / 硬编码凭证两形态），共享"Docker 无浏览器 24h 稳定运行"判定测试。
- **反模式 24 → 7**（signer 生命周期）：「每请求重建沙箱重置计数器」并入「并发共享污染」，统一为"把有状态 signer 当无状态函数用"的两种违反形态（共享串扰 / 重建重置），处置合并为"单例常驻跨请求复用，高并发实例池——不共享也不重建"。
- **反模式 12 → 11**（测量优先）：「未测检测项断言需完整浏览器」并入「外部失败未验证签名内容归因通道层」，统一为"未测量就归因/断言"三形态（过期样本 / 表象归因 / 未测检测项），对齐探针法 diff 清单为统一处置与判定测试。
- **反模式 14、15 → 13**（实验纪律）：「多变量 / 实验代码 bug 不自查」并入「惩罚期做实验」，统一为"实验有效性纪律"九条（先验基线 + 失败预算 + 单变量 + 自检 + 阳性对照 + 成功假象 + 频率墙证伪 + 实验矩阵 + 自查），15 的"服务端魔改 PRNG"记录保留。
- **反模式 26 → 23**（走错分支）：「分支漂移·格式全对但被拒」并入「签名含不可复算随机量仍枚举算法」。2.3.79 曾以"信号不同"为由分列，本轮按文件自身"同源（都是走错分支）"的判定合并为**双形态条目**——共同根因相同，差异只在信号可辨识度（随机量一眼可见 / 格式全对易误归因），两形态的实战案例与全部处置（回溯分支条件、补宿主对象见规则 26、两组样本反证、禁静默吞错、排查顺序）均保留。
- **结构调整**：速查表 27 行 → 19 主条目 + 1 合并指针行（恢复"约 20 行"扫视承诺）；散落各处的 8 个指针条目详情收敛为文末单个「已合并条目指针」索引表；"贡献新反模式"规则更新为"实条 ≤20、新增编号前先过同根因检查"。
- 验证：全仓 78 处文件外「反模式 N」编号引用逐一走指针映射核对，**0 悬空**（含 SKILL.md §9 引用的 22/23/24/25/26/27、experience-rules 引用的 13/14/16/19/22/26/27、cases 引用的 8/11/17/18/20/23/24/25/27）；文件内部交叉引用同步改指主条目（14→13、24→7）；`check_skill_consistency.js --project-dir .` 通过（131 条目 0 问题）。
...
用户复盘 match12 后确认两条方向：①"源码直读型" trace 不做豁免——无论 trace 对定位有无贡献都必须取证一次，它是流程锚点，开口子会诱发 AI 在静态分析阶段自我说服跳过取证（滑向反模式 16）；②反模式清单 21 条太多，扫视成本高。落实：

- **common-pitfalls.md 重构**（432 行 → 291 行，主条目 21 → 18）：
  - 顶部新增速查表（编号/一句话反模式/一句话正确动作/常发阶段），激活 skill 后只扫这张表，卡住才读详情——取代旧"至少看 1-21 条标题"的扫视方式。
  - **同根因合并**（编号保持稳定不重排，被并入编号保留指针行，旧引用不断链）：反模式 21 并入 11（通道层归因前未验证签名内容，含过期样本与表象归因两种形态）；反模式 5 并入 20（"本步骤卡住 → 滑坡成整体不可行"，20 为含终局修正的完整实战）；反模式 15 并入 14（对照实验纪律统一为单变量 + 阳性对照 + 自查实验代码三件套）。
  - 叙事压缩：每条收敛为"实战案例（含 AI 操作）/为什么不成立/正确做法/判定测试"四段，"AI 的辩护"并入"为什么不成立"。
  - 贡献规则更新：同根因合并优先于新增编号；主条目 25 封顶。
- **外部引用同步**（指向被并入编号的高频引用改为直达主条目）：experience-rules.md、match-index.md、match10 案例（4 处 21→11）、debug-playbook.md（15→14）、toutiao 案例（5→20）；CHANGELOG 历史记录不改。git grep 验证 5/15/21 非指针残留引用为零，check_skill_consistency 通过。
- **trace-flow.md 固化"源码直读型也不豁免"决策**：Step 2 价值不只是破案证据更是流程锚点（防反模式 16 滑坡）；简单 case 的 trace 成本是流程固定成本，不是可优化项。
...
用户复盘 match10 的 trace 结果分析流程，对照现有规则发现四处空白（现有规则覆盖了多进程合并与调用栈过滤，但停留在「怎么做统计」，未覆盖实际破案的关键跃迁）：

- **trace-flow.md TRACE_ANALYZE 重构为双轨**：
  - 频次轨（第 1 步）扩展两层过滤（主样本识别——部分进程日志可能全部来自第三方脚本；调用栈过滤）+ 访问类型分组用途（GET/CALL 频次→补环境优先级；SET 高频→状态写入/指纹采集通道，需带内部状态的值管理实现，静态 stub 顶不住高频写入语义）。
  - 新增关键事件轨（第 2 步）：**高频 ≠ 破案关键**（match10：最高频 cookie setter 数千次只是常规写入，破案钥匙是低频 eval 捕获）；三类关键事件低频也必查——eval/Function 动态代码捕获、writer/加密入口调用栈、动态资源请求前后调用序列。
- **trace-flow.md 新增「eval / Function 动态代码捕获的三种用途」小节**：①动态代码收集（引导器流式解密拼出的核心代码静态下载拿不到，eval 捕获是唯一渠道，保存进 case/js/original|extracted 作为算法分析输入——match10 核心 238KB 完整来自 eval 捕获）；②运行时状态基准对照（eval 出的完整配置与 sandbox 同名状态 diff，比 Proxy 探测盲找快）；③完整配置提取（运行时聚合后的最终形态，比静态逆向拼凑可靠）。
- **trace-flow.md RuyiTrace 优先诊断原则第 4 条**证据清单补一项：eval 捕获与 sandbox 状态的 diff（状态残缺最快定位方式）。
- **final-summary.md 生产级「环境与指纹 API 调用回放明细」**章节补推荐表结构：按环境模块分组，四列——API（含访问类型）/ trace 频次 / 回放值或实现方式 / 验证结果，trace 证据、补环境实现、请求验证三者闭环（match10 形态）。
- 案例文档相关参考表补 trace-flow.md 双向链接。
