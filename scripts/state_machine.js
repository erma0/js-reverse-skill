...
// 3) --guard replay 做动作边界守卫：重放/写请求类联网入口只能在 REAL_VERIFY / DIAGNOSE 执行，
//    前置阶段（TRACE_CAPTURE/CASE_LOOKUP/IDENTIFY/TRACE_ANALYZE/IMPLEMENT）越权重放会被拒绝。
...
function usage() {
  ...
  node scripts/state_machine.js --case-dir <case-dir> --get [--json]                                   # 查看当前状态与历史
...
function main() {
  ...
  if (args.guard) {
    if (args.guard !== 'replay') {
      console.error(`未知守卫类型：${args.guard}（当前支持 replay）`);
      return 1;
    }
    const allowed = REPLAY_NODES.includes(state.node);
    if (!allowed && !args.force) {
      const msg = `当前节点 ${state.node} 禁止发起重放/写请求；该动作只能在 ${REPLAY_NODES.join(' / ')} 执行（先完成 TRACE_ANALYZE/IMPLEMENT 写出实现）`;
      state.blocks = (state.blocks || []).concat({ at: new Date().toISOString(), type: 'replay-guard', node: state.node, message: msg });
      writeState(caseDir, state);
      if (args.markdown) console.log(renderMarkdown(state, ['', `> 拒绝：${msg}`, '', `> ${suggestNext(state)}`]));
      else console.error(`REPLAY_GUARD_DENIED: ${msg}`);
      return 2;
    }
    if (!allowed) {
      state.blocks = (state.blocks || []).concat({ at: new Date().toISOString(), type: 'replay-guard-force', node: state.node, message: '--force 放行越权重放' });
      writeState(caseDir, state);
    }
    console.log(args.markdown ? `**replay 守卫通过**：当前节点 ${state.node}` : `replay 守卫通过：当前节点 ${state.node}`);
    return 0;
  }
