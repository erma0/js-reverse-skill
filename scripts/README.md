...
| `capture_ruyitrace_log.js` | 自动采集或手动导入 RuyiTrace NDJSON；默认采集窗口 120 秒，trace 信号命中后自动收尾，等待末行完整刷盘，记录 `endReason` 并校验 Firefox 进程已退出；关闭与导入会使命令总耗时略长于窗口 | `node scripts/capture_ruyitrace_log.js --url <目标URL> --case-dir <project-root> --trace-signal handshake --ruyitrace-home <RuyiTrace目录> --import-after --markdown` |
