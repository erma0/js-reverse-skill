...
| 200KB+、while-switch、dispatcher、字节码数组 | JSVMP 黑盒执行或最小环境复现，不反编译；**先扫字节码尾部大数字面量序列（如 `m324665p2098959o9832905...` = JSBN 28-bit limbs / RSA 模数 / 算法常数）判断标准算法族，命中 RSA/AES/哈希且 padding 确定（如固定 0x01）即转纯算**——match28 实证：确定性 RSA-1024 可字节码 limbs 直读 + BigInt 模幂纯算，完全不用跑 VM（规则 35） |
