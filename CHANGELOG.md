...
### §9/§10 与 §4.1/§4.4 判据密集区精炼（32639 → 32463 字符）

- `SKILL.md`：阶段动作边界与「IMPLEMENT 前置条件」等已外迁段的状态行示例删除（模板保留）；§4.1 目录树由 8 行压为 5 行（保留全部目录名与门禁依赖说明）；§9 路径 B/C/D 的 bullet 合并去重（症状→编号映射全保留）；§10 双对照协议删 hook 库层降级等已有 references 承载的操作细节（判据与 `check_risk_layer_diagnosis.js` 裁定保留）
- 说明：本批为 §7-13 骨架化的收尾——SKILL.md 经 2.3.123/124/141 三轮外迁后，剩余区已是判据密集区（每删一句都有锚点/门禁/判据可见性代价），进一步下探需把 R1 判据本体外迁（不推荐，会失「不读文档也能执行」保证），因此本轮体积优化止于 32463
- 验证：check_skill_consistency 0 问题；check_routing_benchmarks 41/41；state_machine --self-test PASS
