...
4. **坑：末页（page=5）数据返回为空** → 末页 UA 必须为 `yuanrenxue`（平台共性红线），仅末页生效，其余页普通 UA 即可。
5. **坑：提交接口按 JSON 发送被拒** → 必须表单编码（本题 submit 逻辑取证于 document.html 的 `$.ajax({data:{...}})`）。
...
4. 数据绑定 sessionid：同 sessionid 多轮运行数字序列完全相同（可作签名正确性自检手段）
