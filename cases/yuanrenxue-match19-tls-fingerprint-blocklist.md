...
- 末页 UA 分流：page=5 普通 UA → HTTP 200 + 提示数组 `["请","将","UA","改","为","yuan","ren","xue","哦"]`（非 4xx，须按 data 元素类型判别）；UA=`yuanrenxue` → 真实数据
- 风控特征：数据**绑定 sessionid**（跨 7 个浏览器会话 + 3 轮协议取数逐字节恒定）
...
1. **坑（本题最大认知陷阱）："token failed" 文案 ≠ 令牌参数缺失**。400 文案与 match9 的 m-cookie 机制同字面（"缺失→400 token failed"），极易顺着"找 token 参数"方向去逆 `m`——而真实请求里根本没有 token。正确做法：先跨客户端栈对照（同参数分别用 Node/curl/Python 发），curl 与 requests 200 而 Node 400 → 结论是**传输层客户端指纹黑名单**，与任何参数无关。见规则 27 的跨栈对照法。
...
3. **坑：Node 交付路径被封死时的语言切换要有证据链**。Node https（http1.1）与 Node http2 双双 400 后，不要立刻给 Node 加 TLS 指纹伪装（脆弱且不可持续）——先证明"问题出在客户端栈"（curl/requests 同参数 200），再按 skill 规则切换交付语言（Python requests 的 `session.get/close` 同样满足 Session 三件套门禁）。
4. **坑：末页提示数组是 200 不是 4xx**。UA 校验未过时返回 `{"data":["请","将",...]}`——判别方式是**检查 data 元素类型**（`typeof v !== 'number'` 即提示数组），不能只看状态码；实现里要把该形态显式报错并提示 UA 要求，避免把提示数组当数据求和。
...
3. 数据接口 `GET /api/question/19?page=N&pageSize=10&kw=`，响应 `{"data":[10 个 6 位整数]}`，pages 1-4 任意正常浏览器 UA；page 5 必须 `User-Agent: yuanrenxue`
