...
- [x] 蜜月期/惩罚期风控（开窗/续期/惩罚冷却）与第 6 题同源底座
- [x] 无强制页间隔（实测无 sleep，仅签名器自身 ~1s 节奏 5/5 页全过，与浏览器连续翻页一致）
...
7. 蜜月期/惩罚期与 match4/6 同源底座；无强制页间隔（实测无 sleep 全过）
8. 惩罚期表现：连续失败后全拒 token failed（与签名错同文案，不区分病因）
