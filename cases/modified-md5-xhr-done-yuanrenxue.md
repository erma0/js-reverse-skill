...
- [x] 无强制页间隔（实测去掉 sleep 仅靠签名器自身 ~1s 节奏 5/5 页全过，与浏览器连续翻页一致）；连续失败 5-10 次进惩罚期（全拒 token failed，冷却 10 分钟+）
...
7. 蜜月期：GET HTML 开窗约 5 秒，成功请求续期；无强制页间隔（签名正确前提下，实测无 sleep 全过）
