...
- 核心思路：`m = 二次URL编码(base64(RSA-1024-PKCS1v1.5("1|" + t)))`，q 单段链 `1-<t>|`，m 内 t 与 q 末段 t 必须同源；先 GET HTML 开蜜月期窗口，再逐页请求（间隔 2.5s，末页 UA=yuanrenxue）
...
1. **蜜月期窗口**：GET `/match/6`（HTML）为 session 开启约 5 秒窗口；窗口内签名请求放行，成功请求续期；窗口外返回风控文案
...
4. **频率层**：页间隔过快触发风控（≥ 2 秒稳定）
5. **惩罚层**：连续失败约 5-10 次后 session 进入惩罚态（连页面自身请求也被拒），冷却约 10 分钟以上恢复
...
8. 蜜月期：GET HTML 开窗约 5 秒，成功请求续期；页间隔 ≥2 秒稳定
9. 惩罚期：连续失败 5-10 次触发，冷却 10 分钟+恢复，期间连浏览器请求也被拒
10. 风控文案 `{"0.0": "风控不通过，别匆忙，放慢速度"}` 不区分具体原因（签名错/超窗/频率/惩罚全同文案）
