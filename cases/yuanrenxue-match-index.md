...
| 10 | 瑞数 v3 变种 + 重放攻击对抗 ⚠️反例 | rs.js(118KB) + api2/10($_ts 318KB 字符串表) + 内联组装脚本(73KB obfuscator.io「代码组装器」) + /api/10/offset 动态随机数 + meta challenge 拼装 m 生成链；m 由瑞数 hook XHR.open 自动注入（页面传 token=undefined 最终 URL 是 m=...），实时生成**绑定 page**（m1 复用给 page2/3/4 全 400）；不同接口 m 值不同前缀相同（RSA 分块）；数据**绑定 sessionid**（服务端页面加载时重置 sessionid，复用取证 profile 保留 session 才能拿用户数据，答案 29178743）；末页 UA=yuanrenxue；提交接口 `POST /a/10` **表单编码**（jQuery data 默认，非 JSON）；反调试：Function("debugger") 循环、eval.toString 检测、W5/W3 控制流空转。**本次违规用浏览器黑盒取数，正确路径应走 B vm 沙箱补环境 + 对齐探针法** | `cases/yuanrenxue-match10-ruishu3-replay-defense.md` | ⚠️违规（浏览器黑盒取数，应走 B vm 沙箱补环境） | 2026-08-25 |
...
- `references/network/dynamic-resource.md`：黑盒加密 SDK 定期更新专节（公钥随版本轮换 + 二进制抓取纪律，match/9 核心，match10 rs.js/api2 同理）。
- `references/env/env-detect-bypass.md`：瑞数 v3 环境检测对齐探针法（match10 应走未走的正确路径）。
...
- `references/workflow/common-pitfalls.md`：反模式 16（补环境死循环诊断，含插桩 while(1) 禁令，match10 实测）+ 反模式 19（数据差异未先验 session 基线，match9/10 实测）+ 反模式 20（VM 卡死后转投浏览器黑盒取数，match10 为主体）。
