...
- 请求特征：`GET /api/question/16?page=N&m=...&t=...`；Cookie 侧无签名（仅 `sessionid` + 百度统计 `Hm_lvt_*`）；page5 UA=yuanrenxue（平台通用）
...
3. **坑：page5 仍用普通 UA → HTTP 200 + 中文提示数组，只看状态码会误判通过**（平台通用形态）。正确做法：page5 用 `UA=yuanrenxue`，并校验 `data` 元素类型（数值数组才算通过）。
...
6. page5 普通 UA 返回提示数组（HTTP 200、data 非数值）；`UA=yuanrenxue` 返回 10 个数值
...
8. 接口路径：老版 `/api/match/16` 已改 `/api/question/16`——JS 内仍保留老常量，由模块 58 的 `$.ajax` hook 改写（本题实测证据；接口版本迁移通用告诫见平台篇）
9. 提交 `POST /a/16` 表单编码（本 case 未做提交实测）
